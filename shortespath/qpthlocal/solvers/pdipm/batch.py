...
def sp_lu_solve(b,*arg):
    from scipy.linalg import lu_factor, lu_solve

    batchsize = len(b) if len(b)>0 else 1
    sol = []
    b_np = b.detach().numpy()
    if b_np.ndim==2:
    ...
    for b in range(batchsize):
        ...
        cnt=0
        for l in arg:
            logging.info("nan in arg {}".format(np.isnan(l.tolist()).any()))
            # logging.info(l)
            lu= lu + ((l[b]-cnt).detach().numpy(),)
            cnt+=1
        # lu_solve(lu,b_np[b])
        sol.append(lu_solve(lu,b_np[b]))
        logging.info("nan in sol {}".format(np.isnan(sol).any()))
...
def sp_lu_hack(A):
    ...
    from scipy.linalg import lu_factor, lu_solve
    batchsize = len(A) if len(A)>0 else 1
    if A.ndimension()==2:
        A = A.unsqueeze(0)
    lu_lst = []
    piv_lst = []
    ...
    for b in range(batchsize):
        A_np = A[b].detach().numpy().squeeze()
        np.fill_diagonal(A_np, A_np.diagonal() + damping_value)
        logging.info("let's do factorization in scipy ")
        lu, piv = lu_factor(A_np)
        lu_lst.append(lu)
        piv_lst.append(piv+1)
        if np.any(np.isnan(lu_lst)):
            logging.info("sp hack making nan too")

    

    return (torch.tensor(lu_lst),torch.tensor(piv_lst))

# def lu_hack(x):
#     damping_value = 1e-5
#     x_damped = x+ damping_value* torch.eye(len(x),dtype= torch.float)
#     data, pivots = x_damped.lu(pivot=not x.is_cuda)
#     # if x.is_cuda:
#     #     if x.ndimension() == 2:
...
#     #     else:
#     #         assert False
#     # if any(torch.isnan(torch.flatten(data)).tolist()):
#     #     logging.info("as nan calling sp hack")
#     #     return sp_lu_hack(x)
#     return (data, pivots)
