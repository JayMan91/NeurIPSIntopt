...
def IPOfunc(A =None,b =None,G=None,h=None,alpha0=0.9995,beta=0.1,pc = True,
    tol= 1e-8,max_iter=1000,init_val= None,method =1,mu0=None,damping= 1e-2,
    ...
    class IPOfunc_cls(Function):
        ...
        def forward(ctx,c):
            ...
            if not complete:
                ...
                x_v = torch.from_numpy(x_hat).float()
                y_v = torch.from_numpy(y_hat).float()
                t_v = torch.from_numpy(t_hat).float()                
                #print("y full",y_hat)
                # two choices
        ...
        def backward(ctx,del_x):
            ...
            #     dA[i,:] = -y[i]*del_x*x**2/mu

            # for i in range(dG.shape[0]):
            #     dG[i,:] = -z[i]*del_x*x**2/mu
            #### the above is not in our focus ###
            ### calculate c only for inequaliy and equality constraints
            ...
            # X2 = torch.diagflat(x**2)
            # grad = X2
            x2= x**2
            # for i in range(G.shape[0]):
            #     print("inside G loop{:%Y-%m-%d %H:%M:%S}".format(datetime.datetime.now()))
            #     gk = G[i,:]
            #     vec = torch.matmul(X2, gk)
            #     print("matmul finished{:%Y-%m-%d %H:%M:%S}".format(datetime.datetime.now()))
            #     num = torch.einsum('i,j->ij', vec, vec)
            #     print("einsum finished{:%Y-%m-%d %H:%M:%S}".format(datetime.datetime.now()))
            #     den = (x.dot(gk) -h[i])**2 + vec.dot(vec)
            #     print("denomintor done{:%Y-%m-%d %H:%M:%S}".format(datetime.datetime.now()))
            #     grad = grad - (num/den)
            # dc = -torch.matmul(grad, del_x)/mu
            
            if len(A) >0:
                # M =  torch.matmul(torch.matmul(A,grad), torch.t(A) )
                a = A*x
                M = torch.matmul(a,a.T)
                # M  = torch.matmul(A,(x2.view(-1,1)*torch.t(A)))
                if any(torch.isnan(torch.flatten(M)).tolist()):
                    logging.info("nan in M")


                # r = -torch.matmul(A,grad)
                # r = -A*x2 #old
                r = A*x2
                if any(torch.isnan(torch.flatten(r)).tolist()):
                    logging.info("nan in r")

                # e1,e2 = torch.eig(M,eigenvectors=True)
                #logging.info("eigen values of grad%s"%( torch.diagonal(e2,0)))
                try:
                    # factor = torch.cholesky(M)
                    # v = torch.cholesky_solve(r, factor)
                    ...
                    ## will it be M????###
                    v_numpy = sp.linalg.solve( M_numpy ,r_numpy ,assume_a='sym')
                    v = torch.from_numpy(v_numpy).float()

                    # M_LU = torch.lu(M)

                    # v = torch.lu_solve(r, *M_LU) # Problematic 
                    # end_torch = time.time()
                    # print("torch time",end_torch- start_torch)

                    
                    # if any(torch.isnan(torch.flatten(v)).tolist()):
                    #     logging.info("nan in v")
                    #     raise Exception("Aborted:c contains  inf, nan, or None" )
                    #     # M_numpy = M.detach().numpy()
                    #     # r_numpy = r.detach().numpy()
                    #     # ## will it be M????###
                    #     # v_numpy = sp.linalg.solve( M_numpy ,r_numpy ,assume_a='sym')
                    #     # v = torch.from_numpy(v_numpy).float()
                    #     # logging.info("nan in v after numpy?  {}".format(any(torch.isnan(torch.flatten(v)).tolist())))                      
                                  
                except Exception as e:
                    raise (LinAlgError)                    
                    # torch.set_printoptions(profile="full")
                    # # e1,e2 = torch.eig(grad,eigenvectors=True)
                    # # logging.info("eigen values of grad%s"%( torch.diagonal(e2,0)))
                    # logging.info("failed")
                    # torch.set_printoptions(profile="default")
                # u = -torch.matmul(grad, (torch.eye(n)+ torch.matmul(torch.t(A),v)))

                # u = -x2.view(-1,1)* (torch.eye(n)+ torch.matmul(torch.t(A),v)) #old
                u = -x2.view(-1,1)* (torch.eye(n)- torch.matmul(torch.t(A),v))
                # (x2.view(-1,1)*torch.t(A))
                if any(torch.isnan(torch.flatten(u)).tolist()):
                    logging.info("nan in u")

                u_cut = u[0:n_var,0:n_var]
                dc = torch.matmul(u_cut, del_x)/mu
            # print("dc ",dc)
